...
import {getElementByLang} from "../../../../all/dict/Dicts";
import {change} from "redux-form";

const cookies = new Cookies();

if (cookies.get('jwt') === null || cookies.get('jwt') === undefined) {
    document.location.href = '/public/en/?signin=1'
}
...
function loadArtist(userSession, locale, store, history){
    axiosBackendApi.get('users/'+userSession.user.id+'/artists', {
        ...
        .then(result => {
            ...
            let personalInfo = getElementByLang(result.data.listPersonalInfo, locale)
            store.dispatch(change("PictureNewCardForm", "author", personalInfo.firstName+" "+personalInfo.lastName));
        })
        .catch((error) => {
...
function indexStartup(locale, theme, store, history, userSession) {
    axios.get('/private/assets/newvalue/pict/' + locale + '.json')
        .then(result => {
            if (result.status >= 400) {
                throw new Error('World Art Registry temporarily unavailable.')
            }
            return result.data;
        })
        .then(localeData => {
